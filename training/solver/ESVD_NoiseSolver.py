...
class SVD_Embedding_Solver:
    def __init__(
        ...
        self.unet_embedding = NoiseTransformer(resolution=512//8).to(device).to(torch.float32)
        self.unet_svd = SVDNoiseUnet_wo_attention(resolution=512//8).to(device).to(torch.float32)
        ...
        for epoch in range(1, total_epoch + 1):
            ...
            for step, (original_noise, optimized_noise, prompt) in enumerate(pbar, 1):
                    ...
                    (
                        prompt_embeds,
                        _,
                    ) = self.pipeline.encode_prompt(prompt=prompt[0], device=self.device, do_classifier_free_guidance=True, num_images_per_prompt=1) # prompt[0]
                    ...
                    golden_noise = self.unet_svd(encoder_hidden_states_svd.float()) + (2 * torch.sigmoid(self._alpha) - 1) * text_emb + self._beta * golden_embedding
    ...
    def generate(self,
        ...
        with torch.no_grad():
            ...
            (
                prompt_embeds,
                _,
            ) = self.pipeline.encode_prompt(prompt=prompt, device=self.device,  do_classifier_free_guidance=True, num_images_per_prompt=1) # prompt[0]
